...
class TestMain(unittest.TestCase):
    ...
    @patch("src.main.time.sleep", side_effect=KeyboardInterrupt)  # Simulate exiting the loop
    def test_update_and_publish_data(self, name, server_config, current_hour, should_update, mock_sleep, mock_publish_to_mqtt, mock_update_data):
        ...
        with patch("src.main.time.localtime", return_value=MagicMock(tm_hour=current_hour)):
        ...
        if should_update:
            ...
            mock_publish_to_mqtt.assert_called_once_with(server_config, mqtt_config, {"key": "value"})
        else:
